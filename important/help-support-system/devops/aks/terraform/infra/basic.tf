data "azurerm_resource_group" "hss_rg" {
  name     = var.rgName
}
...
data "azurerm_user_assigned_identity" "nodepoolIdentity" {
  resource_group_name = azurerm_kubernetes_cluster.hssCluster.node_resource_group

  name = "${azurerm_kubernetes_cluster.hssCluster.name}-agentpool"
}
